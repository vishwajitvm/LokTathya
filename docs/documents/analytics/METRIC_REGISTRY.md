...
| Domain | ANALYTICS |
| Subdomain | METRIC_REGISTRY.MD |
| Status | ARCHITECTURAL |
| Version | 1.0.0 |
| Last Updated | 2026-08-22 |
| Filename | METRIC_REGISTRY.md |

---

## 1. Purpose
This document specifies the technical requirements, design constraints, operational details, and architectural integration rules of the `METRIC_REGISTRY.md` component within the LokTathya platform. It acts as an authoritative reference for developers and system auditors.

## 2. Scope
This specification covers the implementation details of `METRIC_REGISTRY.md` inside the `analytics` subsystem. It defines data structures, API contracts, processing pipelines, and validation requirements, ensuring consistency across both local containerized dev environments and production cloud architectures.

## 3. Problem Statement
The LokTathya civic intelligence system requires robust, isolated mechanisms to manage `METRIC_REGISTRY.md` capabilities. Without structured, verifiable constraints, data processing and API integrations in this area can lead to inconsistencies, performance bottlenecks, and validation drift. This document addresses these concerns by establishing clean boundaries and validation rules.
...
The architectural layout of `METRIC_REGISTRY.md` is built around decoupled service boundaries. The system utilizes Celery tasks for background processing, PostgreSQL for relational storage, Redis for caching state, and FastAPI for routing requests. In the frontend, Next.js page components interact with backend routes via typed clients.

## 7. Data Model & Schema Details
All tables related to this domain are prefixed with `ana_` to ensure logical isolation. Schema columns enforce strict primary key limits, foreign key check constraints, and B-tree indexing on lookup fields. Geometries use SRID 4326 and are indexed using spatial GIST parameters.
...
Deterministic metrics are calculated using verified algebraic formulas. All ratios (e.g. attendance rates, project completion rates) are computed using database aggregations rather than LLM projections. Financial indicators are indexed for inflation based on official MoF indexes.

## 17. Related Documents
* [DETERMINISTIC_METRICS.md](DETERMINISTIC_METRICS.md)
* [PUBLIC_FINANCE.md](../../features/04-finance/PUBLIC_FINANCE.md)